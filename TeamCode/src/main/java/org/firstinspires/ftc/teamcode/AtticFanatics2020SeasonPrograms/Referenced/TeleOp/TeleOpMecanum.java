...
public class TeleOpMecanum extends Configure {
    ...
    public void Move(HardwareMap ahwMap, Gamepad G1, Gamepad G2)
    {
        ...
        else if(G1.right_bumper) RampSpeed(G1.left_stick_x, G1.left_stick_y, G1.right_stick_x, 1); // RampSpeed at normal speed
        else if(Math.abs(G1.left_stick_x) >= 0.1 & Math.abs(G1.left_stick_y) < 0.1) setPower(G1.left_stick_x, 0, 0); //Enables easier direct sideways movement
        else setPower(G1.left_stick_x, G1.left_stick_y, G1.right_stick_x); //Normal move, no bells and whistles here

        if(!G1.left_bumper && !G1.right_bumper) RampDiff = INPUT_RAMP_DIFF; // Reset ramping after stopping ramping
